...
from .F3Net_Res2Net import Res2Net_v1b_26w_4s
import math
from .F3Net_Res2Net import Bottle2neck
import numpy as np


class BasicConv2d(nn.Module):
    def __init__(self, inChannel, outChannel, kernelSize, stride=1, padding=0, dilation=1):
        super(BasicConv2d, self).__init__()
        self.conv = nn.Conv2d(inChannel, outChannel, kernelSize, stride, padding, dilation, bias=False)
...
class F3Net(nn.Module):
    def __init__(self, opt, channel=32):
        super(F3Net, self).__init__()
        self.rf3resnet = Res2Net_v1b_26w_4s(opt)
    ...
    def forward(self, input):
        x = self.rf3resnet.stem(input)  # layer1
        out2h = self.rf3resnet.layer1(x)
        out3h = self.rf3resnet.layer2(out2h)
        out4h = self.rf3resnet.layer3(out3h)
        out5v = self.rf3resnet.layer4(out4h)
