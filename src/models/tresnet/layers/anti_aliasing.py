...
class DownsampleJIT(object):
    ...
    def __call__(self, input: torch.Tensor):
        ...
        self.filt = self.filt.cuda(input.get_device()) # 防止filt不在同一个gpu上
        rnt = F.conv2d(input_pad, self.filt, stride=2, padding=0, groups=input.shape[1])
        return rnt
...
class Downsample(nn.Module):
    ...
    def forward(self, input):
        input_pad = F.pad(input, (1, 1, 1, 1), 'reflect')
        rnt = F.conv2d(input_pad, self.filt, stride=self.stride, padding=0, groups=input.shape[1])
        return rnt
